...
//  Skiptaculos
//
//  Created by Oscar Calles on 25/07/14.
...
class PlayScene: SKScene, SKPhysicsContactDelegate {
    ...
    override func didMoveToView(view: SKView!) {
        ...
        blockStatuses["block1"] = BlockStatus(isRunning: false,
                                  timeGapForNextRun: ramdom(),
                                  currentInterval:UInt32(0))
        blockStatuses["block2"] = BlockStatus(isRunning: false,
                                  timeGapForNextRun: ramdom(),
                                  currentInterval:UInt32(0))
    ...
    func blockRunner(){
        for(block, blockStatus) in self.blockStatuses {
            ...
            if blockStatus.shouldRunBlock() {
                blockStatus.timeGapForNextRun = ramdom()
